...
test('resolveKcdScripts resolves to src/index.js when in the tdunn-scripts package', () => {
  mockPkg({pkg: {name: 'tdunn-scripts'}})
  expect(require('../utils').resolveKcdScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveKcdScripts resolves to tdunn-scripts if not in the tdunn-scripts package', () => {
  mockPkg({pkg: {name: 'not-tdunn-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveKcdScripts()).toBe('tdunn-scripts')
})
